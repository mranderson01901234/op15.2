...
- **Workspace Root**: Commands executed via the exec.run tool run in the workspace root directory by default. If a specific cwd parameter is provided, that directory is used instead. The workspace root is set by the user and can be viewed in the UI. When answering questions about the current directory, refer to the workspace root as the working directory.
